...
const COUNTRIES = [{ code: "+216", flag: tunisiaFlag, name: "Tunisia" }];
        ...
        >
          {COUNTRIES.map((c) => (
            <div
              ...
              style={{
                padding: ".55rem .75rem",
                paddingLeft: hovered === c.code ? "1.1rem" : ".75rem",
                fontSize: ".85rem",
                display: "flex",
                alignItems: "center",
                gap: ".55rem",
                color:
                  hovered === c.code ? "var(--rider-fg)" : "var(--text-body)",
                background:
                  hovered === c.code ? "var(--rider-bg)" : "var(--bg-card)",
                cursor: "pointer",
                transition:
            ...
            >
              {!imgErr.has(c.code) ? (
                <img
                  src={c.flag}
                  alt={c.name}
                  onError={() => setImgErr((p) => new Set(p).add(c.code))}
...
function parsePhone(raw: string): { dialCode: string; digits: string } {
  for (const c of COUNTRIES) {
    if (raw.startsWith(c.code))
      return { dialCode: c.code, digits: raw.slice(c.code.length) };
  }
...
export default function EditModal({ user, onClose, onSave }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    role: (user.role === "driver" ? "driver" : "passenger") as
      | "passenger"
      | "driver",
  });
  ...
  function validate() {
    ...
    if (form.phoneDigits.trim()) {
      const digitsOnly = form.phoneDigits.replace(/\s/g, "");
      if (digitsOnly.length !== 8) {
        e.phone = "Phone number must be 8 digits (format: xx xxx xxx)";
      }
    }
    return e;
  }
        ...
        <div className="ts-modal-header">
          <div>
            ...
            <p className="ts-page-subtitle">
              Update details. Use Block/Unblock to manage access.
            </p>
          </div>
